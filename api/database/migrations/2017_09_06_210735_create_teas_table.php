...
class CreateTeasTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('teas', function (Blueprint $table) {
            ...
            $table->date('delivery_time')->comment('实际交付日期');
            $table->date('delivery_time_deadline')->comment('交付时间截止日期');
            $table->unsignedTinyInteger('is_deliveried')->default(0)->comment('是否已交付');
            $table->string('remark', 255)->default('')->comment('交付备注, 例如: 交付数量、地址等备注信息');
            $table->integer('updator_id')->default(0)->comment('修改人ID');
            $table->string('updator_name', 30)->default('')->comment('修改人名称');
            $table->softDeletes();
            $table->timestamps();
        });
    }
